<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exist="http://exist.sourceforge.net/NS/exist" xmlns:telota="http://www.telota.de" xmlns:tei="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="tei exist telota" version="2.0">
    ...
    <xsl:template mode="kritischerText lesetext faksimileText" match="tei:p">
        <xsl:element name="p">
            <xsl:choose>
                <xsl:when test="./@rendition='#mPrint'">
                    <xsl:attribute name="class">mPrint</xsl:attribute>
    ...
    <xsl:template mode="#all" match="tei:p[@rendition='#mMM']">
        <p class="randstrich">
            <xsl:apply-templates mode="#current"/>
        </p>
    </xsl:template>
    ...
    <xsl:template mode="lesetext" match="//tei:closer//tei:lb">
        <br/>
    </xsl:template>
    ...
    <xsl:template mode="kritischerText faksimileText comment" match="tei:del">
        <span class="del">
            <xsl:apply-templates mode="#current"/>
        </span>
    </xsl:template>
    <xsl:template mode="kritischerText faksimileText comment" match="tei:sic[@ana='#break-off']">
         //
    </xsl:template>
    <xsl:template mode="lesetext" match="tei:del"/>
    <xsl:template mode="kritischerText faksimileText comment" match="tei:add">
        <xsl:choose>
    ...
    <!-- DEPRACATED: ex -->
    <xsl:template mode="kritischerText faksimileText lesetext comment" match="tei:ex">
        <em class="editorergaenzung">
            <xsl:value-of select="."/>
        </em>
    </xsl:template>
    <xsl:template mode="kritischerText faksimileText comment" match="tei:unclear">
        <xsl:choose>
    ...
    <xsl:template mode="kritischerText faksimileText comment" match="tei:gap">
        <xsl:choose>
    ...
    <xsl:template mode="kritischerText faksimileText comment" match="tei:supplied">
        <span class="supplied">[<xsl:apply-templates mode="#current"/>]</span>
    </xsl:template>
    <xsl:template mode="lesetext" match="tei:supplied">
        <xsl:apply-templates mode="#current"/>
    </xsl:template>
    <xsl:template mode="kritischerText faksimileText comment" match="tei:surplus">
        <span class="surplus">
            <xsl:apply-templates mode="#current"/>
        </span>
    </xsl:template>
    <xsl:template mode="lesetext" match="tei:surplus"/>
    <xsl:template mode="kritischerText faksimileText comment" match="tei:choice[./tei:sic]">
        <span class="choice">
            <span class="sic">
            ...
            <xsl:text> </xsl:text><span class="corr">
    ...
    <xsl:template mode="kritischerText faksimileText comment" match="tei:choice[./tei:abbr]">
        <span>
    ...
    <xsl:template mode="kritischerText faksimileText comment" match="tei:choice[./tei:reg]">
        <span>
    ...
    <xsl:template mode="lesetext kritischerText faksimileText comment" match="tei:quote">
        <span class="quote">
            <xsl:apply-templates mode="#current"/>
        </span>
    </xsl:template>
    ...
    <xsl:template mode="#all" match="tei:bibl[@sameAs and not(ancestor::tei:seg and ancestor::tei:note)]">
        <xsl:choose>
            <xsl:when test="not(starts-with(current()/@sameAs, 'zz'))">
    ...
    <xsl:template mode="#all" match="tei:ref[not(@type='siglum')]">
        ...
        <xsl:variable name="target">
            <xsl:choose>
                ...
                <!-- XXX Hack, um Links auf Briefe in Jahrgangseinführung zu ermöglichen -->
                <xsl:when test="boolean(/tei:TEI[@type='year_intro'])">
                    <xsl:text>../briefe/detail.xql?id=</xsl:text>
                    <xsl:value-of select="@target"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:text>detail.xql?id=</xsl:text>
            ...
            <xsl:otherwise>
                <span class="tooltip">
                    <span class="fussnote">
                        ...
                        <br/>
                        <a class="close" title="Schließen" aria-label="Schließen">
                            Schließen <i class="fa fa-times-circle" aria-hidden="true"/></a>
                    </span>
                    <xsl:apply-templates select="tei:orig" mode="#current"/>
                </span>
            </xsl:otherwise>
