...
let $listOrgs := <ul>{
    for $name in collection($edweb:dataRegister)//tei:orgName[matches(./substring(., 1, 1), $edwebRegister:section)]
    order by edwebRegister:orderLabel($name)
    return
    <li><a href="detail.xql?id={$name/ancestor::tei:org/@xml:id}">
        {if ($name[@type='alt']) then (concat($name, ' &#x2192; ', $name/ancestor::tei:org/tei:orgName[@type='reg']/text())) else ($name)}
        </a></li>}
    </ul>
...
let $header :=  <transferContainer>
                    ...
                    <h1>Register der Firmen - {$edwebRegister:section}</h1>
                    <p>{count($listOrgs/li)} Einrichtungen</p>
                </transferContainer>
